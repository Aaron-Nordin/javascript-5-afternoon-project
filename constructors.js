...
User.prototype.removeSavedPost = function(id) {
  let remove = this.savedPosts.findIndex(p => p.id === id)
  return this.savedPosts.splice(remove, 1)
}
...
User.prototype.changePostRating = function(id, rating) {
  let post = this.savedPosts.filter(p => p.id === id)
  return post[0].rating = rating
}